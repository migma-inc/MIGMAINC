...
/**
 * migma-payment-completed-internal (V15)
 *
 * Internal-only wrapper for server-to-server payment completion calls.
 * It intentionally does not rely on Supabase gateway JWT because the caller is
 * another Edge Function. Access is protected by MIGMA_WEBHOOK_SECRET.
 *
 * Responsabilidades:
...
Deno.serve(async (req) => {
  ...
  const receivedSecret = req.headers.get("x-migma-webhook-secret") || "";

  if (!expectedSecret) {
    console.error("[migma-payment-completed-internal] MIGMA_WEBHOOK_SECRET is not configured");
    return new Response(
      JSON.stringify({ error: "Internal auth is not configured" }),
      { status: 500, headers: { ...CORS, "Content-Type": "application/json" } },
    );
  }

  if (receivedSecret !== expectedSecret) {
    console.warn("[migma-payment-completed-internal] Unauthorized internal call blocked");
    return new Response(
