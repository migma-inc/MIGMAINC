-- Keep test students isolated from production mentors and vice versa.
-- Test mentors are identified by referral_mentors.id, while user_profiles.mentor_id
-- stores referral_mentors.profile_id.

DROP FUNCTION IF EXISTS private.pick_least_loaded_referral_mentor();

CREATE OR REPLACE FUNCTION private.pick_least_loaded_referral_mentor(p_student_email text DEFAULT NULL)
RETURNS uuid
LANGUAGE sql
VOLATILE
SECURITY DEFINER
...
AS $$
  WITH student_flags AS (
    SELECT lower(btrim(coalesce(p_student_email, ''))) LIKE '%@uorak.com' AS is_test_student
  )
  SELECT rm.profile_id
  FROM public.referral_mentors rm
  CROSS JOIN student_flags sf
  ...
  WHERE rm.active = true
    AND (
      (
        sf.is_test_student
        AND rm.id IN (
          '0894668a-2768-46a4-b103-c0f31d3df8af'::uuid,
          '15eaab86-9901-4594-8271-09a7793adaf3'::uuid,
          '1f8c4ab8-8ee4-48e2-8560-17131dd31b11'::uuid
        )
      )
      OR (
        NOT sf.is_test_student
        AND rm.id NOT IN (
          '0894668a-2768-46a4-b103-c0f31d3df8af'::uuid,
          '15eaab86-9901-4594-8271-09a7793adaf3'::uuid,
          '1f8c4ab8-8ee4-48e2-8560-17131dd31b11'::uuid
...
BEGIN
  ...
  selected_mentor_id := private.pick_least_loaded_referral_mentor(NEW.email);
...
REVOKE ALL ON FUNCTION private.pick_least_loaded_referral_mentor(text) FROM PUBLIC;
