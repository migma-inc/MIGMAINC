-- Migration: Schedule split payment part 2 reminder emails
-- Description: Creates a database dispatcher and schedules reminder delivery every 10 minutes
-- Date: 2026-04-06
...
COMMENT ON FUNCTION public.dispatch_split_part2_payment_reminders IS
  'Dispatches reminder emails for split payments whose first payment was confirmed more than 2 hours ago and whose second payment remains pending.';

SELECT cron.schedule(
  'split-part2-payment-reminder',
  '*/10 * * * *',
  $$ SELECT public.dispatch_split_part2_payment_reminders(); $$
);
