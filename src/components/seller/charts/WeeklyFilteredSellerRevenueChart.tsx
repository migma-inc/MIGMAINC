...
export function WeeklyFilteredSellerRevenueChart({ data, titleBase }: WeeklyFilteredSellerRevenueChartProps) {
    ...
    // Seleciona automaticamente a primeira semana com vendas
    useEffect(() => {
        if (data && data.length > 0) {
            const firstActive = data.findIndex(w => w.revenue > 0);
            if (firstActive >= 0) {
                setSelectedWeek(firstActive);
            } else {
                setSelectedWeek(0);
            }
        }
    ...
    useEffect(() => {
        ...
        const chartData = rawSellers
            .map(([name, rev]) => ({
                name,
                revenue: rev,
                percentage: totalWeekRevenue > 0 ? (rev / totalWeekRevenue) * 100 : 0
            }))
