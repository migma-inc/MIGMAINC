...
interface SellerRankChartProps {
    data: { name: string; sales: number; percentage: number }[];
    total: number;
    title: string;
    year?: string | number;
}

export function SellerRankChart({ data, total, title, year = new Date().getFullYear() }: SellerRankChartProps) {
    const chartRef = useRef<HTMLDivElement>(null);
    const rootRef = useRef<am5.Root | null>(null);

    const chartData = data.map(d => ({
        ...d,
        labelTitle: `${d.name}   [bold]${d.sales}[/]`
    }));
