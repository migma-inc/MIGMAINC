...
const OrderTableSection = ({
    ...
    getStatusBadge,
    // setSelectedPdfUrl, - Removed as unused in this version
    // setSelectedPdfTitle, - Removed as unused in this version
    currentPage,
    setCurrentPage,
    ITEMS_PER_PAGE,
...
export function AdminSellerOrders() {
    ...
    useEffect(() => {
        const loadSeller = async () => {
            ...
            try {
                ...
                if (error || !sellerData) {
                    console.error('[AdminSellerOrders] Error loading seller:', error);
                    setSeller(null);
                } else {
                    setSeller(sellerData);
                }
            } catch (err) {
                console.error('[AdminSellerOrders] Error loading seller:', err);
            }
        };
        ...
        <div className="min-h-screen bg-gradient-to-b from-black via-[#1a1a1a] to-black p-4 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col gap-4">
                    <Link to={`/dashboard/sellers`} className="inline-flex items-center text-sm text-gray-400 hover:text-white transition-colors">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <h1 className="text-2xl sm:text-3xl font-bold migma-gold-text">Orders - {seller.full_name || seller.email}</h1>
                            <p className="text-sm text-gray-400 mt-1">Viewing orders for Seller ID: {seller.seller_id_public}</p>
                        </div>
                        <Link to={`/dashboard/sellers/${seller.seller_id_public}/analytics`}>
