/**
 * EB-2 Recurring Management Page
 * Admin dashboard for managing EB-2 monthly maintenance installments
 */
...
export const EB2RecurringManagement = () => {
    ...
    const [programs, setPrograms] = useState<EB2ProgramSummary[]>([]);
    const [loading, setLoading] = useState(true);
    const [filterStatus, setFilterStatus] = useState<string>('all');

    // Quick Status Toggle States
    const [isStatusDialogOpen, setIsStatusDialogOpen] = useState(false);
    const [selectedProgramForStatus, setSelectedProgramForStatus] = useState<EB2ProgramSummary | null>(null);
    ...
    const loadData = async () => {
        try {
            ...
            const { data: controlData, error: controlError } = await supabase
                .rpc('get_eb2_program_summaries');

            if (controlError) {
                console.error('[EB-2 Dashboard] Error loading programs:', controlError);
                throw controlError;
            }
    ...
    const handleToggleStatus = async () => {
        ...
        try {
            ...
            const { error } = await supabase
                .rpc('toggle_eb2_recurrence_status', {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 uppercase tracking-tight">EB-2 Recurring Management</h1>
                    <p className="text-gray-400 text-sm sm:text-base">Manage monthly maintenance installments for EB-2 visa clients</p>
                </div>
                <Button
                    onClick={() => loadData()}
                    variant="outline"
            ...
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
                ...
                <Button
                    ...
                    className={cn(
                        "text-xs sm:text-sm whitespace-nowrap px-4",
                        filterStatus === 'completed' ? "bg-gold-medium hover:bg-gold-dark text-black" : "border-gray-700 text-gray-400"
                    )}
                >
                    Completed
                </Button>
                                                ...
                                                <tr key={program.control_id} className="border-b border-gray-800 hover:bg-white/5 transition-colors">
                                                    ...
                                                    <td className="py-4 px-4 text-right">
                                                        <Button
                                                            ...
                                                            onClick={() => navigate(`/dashboard/eb2-recurring/${program.client_id}`)}
                                        ...
                                        <div key={program.control_id} className="bg-zinc-900 border border-gray-800 rounded-xl p-3.5 space-y-3.5 shadow-lg shadow-black/20">
                                            ...
                                            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-800/50">
                                                <div className="min-w-0">
                                                    <p className="text-[9px] text-gray-500 uppercase font-bold tracking-tight mb-1">Progress</p>
                                                    <div className="flex items-center gap-2">
                                                        <p className="text-xs text-white font-bold">{program.installments_paid}/{program.total_installments}</p>
                                                        <div className="flex-1 bg-gray-700/50 rounded-full h-1 overflow-hidden">
                                                            <div
                                                                className="bg-gold-light h-full rounded-full shadow-[0_0_8px_rgba(242,190,34,0.4)]"
                                                                style={{
                                                                    width: `${Math.min((program.installments_paid / program.total_installments) * 100, 100)}%`,
                                                                }}
                                                            />
                                            ...
                                            <div className="flex items-center justify-between pt-3 border-t border-gray-800/50 gap-3">
                                                <div className="flex items-center gap-2.5">
                                                    <span className="text-[9px] text-gray-500 uppercase font-black uppercase tracking-widest">Active</span>
                                                    <CustomSwitch
                                                        checked={program.program_status === 'active'}
                                                        onChange={() => {
                                                            setSelectedProgramForStatus(program);
