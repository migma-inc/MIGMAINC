...
export const EB2RecurringDetail = () => {
    ...
    const loadProgramData = async () => {
        try {
            console.log('[EB2-Debug] Loading data via RPC for client:', clientId);
            setLoading(true);
            setErrorState(null);

            const { data, error } = await supabase
                .rpc('get_eb2_program_detail', { p_client_id: clientId });

            if (error) {
                console.error('[EB2-Debug] RPC Execution Error:', error);
                setErrorState({
            ...
            if (!data) {
                console.warn('[EB2-Debug] RPC returned null for client_id:', clientId);
                setProgram(null);
                return;
            }

            console.log('[EB2-Debug] Data successfully retrieved via RPC:', data);

            // The RPC already returns the format we need
            setProgram(data as ProgramDetail);

        } catch (error: any) {
            console.error('[EB2-Debug] CATCH Error Payload:', {
                message: error.message,
                details: error.details,
                hint: error.hint,
                code: error.code,
    ...
    const loadEmailHistory = async () => {
        try {
            const { data, error } = await supabase
                .rpc('get_eb2_email_history', { p_client_id: clientId });
    ...
    const handleMarkAsPaidManual = async () => {
        ...
        try {
            ...
            const { error } = await supabase
                .rpc('mark_eb2_installment_paid_manual', {
            ...
            setNotification({ message: `Failed to mark as paid: ${error.message}`, type: 'error' });
        } finally {
            setProcessingAction(false);
        }
    };
    ...
    const handleResendLink = async (scheduleId: string) => {
        try {
            ...
            const { error } = await supabase.functions.invoke('send-eb2-installment-email', {
    ...
    const handleCopyLink = (scheduleId: string) => {
        ...
        // EB-2 checkout path
        const paymentLink = `${baseUrl}/checkout/visa/eb2-annex-installment?prefill=${scheduleId}`;
        ...
        return (
            <div className="p-6 text-center max-w-2xl mx-auto">
                ...
                <div className="flex gap-4 justify-center">
                    ...
                    <Button onClick={() => navigate('/dashboard/eb2-recurring')}>
                        Back to Management
                    </Button>
                </div>
            </div>
    ...
    if (!program) {
        return (
            <div className="p-6 text-center">
                ...
                <Button onClick={() => navigate('/dashboard/eb2-recurring')} className="mt-4">
                    Back to Management
                </Button>
            </div>
        );
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    <Button
                        variant="outline"
                        size="icon"
                        onClick={() => navigate('/dashboard/eb2-recurring')}
                        className="border-gray-700 bg-transparent hover:bg-white/10 text-white flex-shrink-0"
                    >
                        <ArrowLeft className="w-4 h-4" />
                    </Button>
                    <div className="min-w-0">
                        <h1 className="text-xl sm:text-2xl font-bold text-white uppercase tracking-tight truncate">EB-2: {program.client.full_name}</h1>
                        <p className="text-gray-400 text-xs sm:text-sm">Monthly EB-2 Maintenance Plan</p>
                    </div>
                </div>
                                                    ...
                                                    emailLogs.map((log) => (
                                                        <tr key={log.id} className="hover:bg-white/5 transition-colors">
                                                            ...
                                                            <td className="py-3 px-4">
                                                                <Badge variant="outline" className="capitalize text-xs text-gold-light border-gold-medium/50 bg-gold-medium/10">
